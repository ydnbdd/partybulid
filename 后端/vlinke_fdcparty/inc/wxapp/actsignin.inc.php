...
if ($op == "display") {
    ...
    $this->result(0, '', array('activity' => $activity, 'branch' => $branch, 'actuser' => $actuser, 'param' => $param, 'user' => $user, 'actenroll' => $actenroll));
} elseif ($op == "setsign") {
    $activityid = intval($_GPC['activityid']);
    ...
    if ($activity['getval'] > 0) {
        $intdata = array('userid' => $userid, 'channel' => "actenroll", 'foreignid' => $actenroll['id'], 'integral' => $activity['getval'], 'remark' => "参与《" . $activity['title'] . "》活动奖励");
    ...
    $this->result(0, '', array('activity' => $activity, 'actenroll' => $actenroll));
}