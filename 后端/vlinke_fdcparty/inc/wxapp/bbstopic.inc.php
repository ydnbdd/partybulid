...
if ($op == "display") {
    ...
    $this->result(0, '', array('topic' => $topic, 'zanarr' => $zanarr, 'zantol' => count($zanarr), 'uzan' => $uzan, 'replyarr' => $replyarr, 'replytol' => count($replyarr), 'ucollect' => intval($ucollect)));
} elseif ($op == "clickcollect") {
    $id = intval($_GPC['id']);
    $userid = intval($_GPC['userid']);
    $bbscollect = pdo_get($this->table_bbscollect, array('userid' => $userid, 'topicid' => $id, 'uniacid' => $_W['uniacid']));
    if (empty($bbscollect)) {
        $data = array('uniacid' => $_W['uniacid'], 'topicid' => $id, 'userid' => $userid, 'createtime' => time());
    ...
    $data = array('uniacid' => $_W['uniacid'], 'topicid' => $id, 'userid' => $userid, 'details' => $details, 'islook' => 0, 'createtime' => time());
