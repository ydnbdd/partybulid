...
if ($op == "display") {
    ...
    $this->result(0, '', array('seritem' => $seritem, 'sercate' => $sercate, 'branch' => $branch, 'myuser' => $myuser, 'userlist' => $userlist, 'usersubmitarr' => $usersubmitarr));
    ...
    if ($serlogtol < $seritem['unumber']) {
        $data = array('uniacid' => $_W['uniacid'], 'userid' => $userid, 'itemid' => $itemid, 'getval' => $seritem['getval'], 'createtime' => time());
        ...
        if ($seritem['getval'] > 0) {
            $intdata = array('userid' => $userid, 'channel' => "serlog", 'foreignid' => $data['id'], 'integral' => $seritem['getval'], 'remark' => "报名志愿服务《" . $seritem['title'] . "》奖励");
