...
if ($op == "display") {
    ...
    $this->result(0, '', array('activity' => $activity, 'branch' => $branch));
} elseif ($op == "getmore") {
    $activityid = intval($_GPC['activityid']);
    ...
    $data = array('uniacid' => $_W['uniacid'], 'activityid' => $activityid, 'userid' => intval($_GPC['userid']), 'details' => $details, 'picall' => iserializer($picallarr), 'createtime' => time());
