...
if ($op == "display") {
    ...
    if (empty($edustudy)) {
        $edustudy = array('uniacid' => $_W['uniacid'], 'userid' => $userid, 'lessonid' => $id, 'getval' => 0, 'status' => 1, 'createtime' => time());
    ...
    if ($edulesson['status'] == 2 && $edustudy['status'] == 1 && count($educhapterall) == count($edulogall)) {
        ...
        if (empty($integral) && $edulesson['integral'] > 0) {
            $data = array('userid' => $userid, 'channel' => "edustudy", 'foreignid' => $edustudy['id'], 'integral' => $edulesson['integral'], 'remark' => "学习《" . $edulesson['title'] . "》课程奖励");
    ...
    $this->result(0, '', array('edulesson' => $edulesson, 'educate' => $educate, 'edustudy' => $edustudy, 'edustudytol' => intval($edustudytol), 'educhapterall' => $educhapterall));
}