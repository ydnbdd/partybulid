...
if ($op == "display") {
    ...
    $this->result(0, '', array('seritem' => $seritem, 'sercate' => $sercate, 'branch' => $branch));
} elseif ($op == "getmore") {
    $itemid = intval($_GPC['itemid']);
    ...
    $data = array('uniacid' => $_W['uniacid'], 'itemid' => $itemid, 'userid' => intval($_GPC['userid']), 'details' => $details, 'picall' => iserializer($picallarr), 'createtime' => time());
