...
if ($op == "display") {
    ...
    if (empty($activity['wxappqrcode'])) {
        ...
        $response = $account_api->getCodeUnlimit($activityid, 'vlinke_fdcparty/pages/act/actsignin', 430, array('auto_color' => false, 'line_color' => array('r' => 0, 'g' => 0, 'b' => 0)));
    ...
    $this->result(0, '', array('activity' => $activity, 'wxappqrcode' => $wxappqrcode, 'branch' => $branch, 'actuser' => $actuser, 'utype1' => $utype1, 'utype2' => $utype2, 'actenrolltol' => $actenrolltol));
    ...
    pdo_update($this->table_activity, array('wxappqrcode' => $filetable), array('id' => $activityid));
    $wxappqrcode = tomedia($filetable);
    $this->result(0, '', array('wxappqrcode' => $wxappqrcode));
} elseif ($op == "setsign") {
    $activityid = intval($_GPC['activityid']);
    $activity = pdo_get($this->table_activity, array('id' => $activityid, 'status' => array(2, 3), 'uniacid' => $_W['uniacid']));
    if (empty($activity)) {
    ...
    $this->result(0, '', array('issign' => $issign));
}