...
if ($op == "display") {
    ...
    if (empty($edulog)) {
        $edulog = array('uniacid' => $_W['uniacid'], 'userid' => $userid, 'lessonid' => $educhapter['lessonid'], 'chapterid' => $id, 'stutime' => 0, 'status' => 1, 'createtime' => time());
    ...
    $this->result(0, '', array('educhapter' => $educhapter, 'edulesson' => $edulesson, 'educate' => $educate, 'edulog' => $edulog, 'educhapterall' => $educhapterall, 'prev' => empty($prev) ? "" : $prev, 'next' => empty($next) ? "" : $next));
} elseif ($op == "stutime") {
