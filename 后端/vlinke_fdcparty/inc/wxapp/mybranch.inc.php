...
if ($op == "display") {
    ...
    $this->result(0, '', array('brancharr' => $brancharr, 'userall' => $userall, 'branch' => $branch));
} elseif ($op == "buser") {
    $id = intval($_GPC['id']);
    ...
    $this->result(0, '', array('buser' => $buser, 'branch' => $branch, 'leader' => $leader));
} elseif ($op == "details") {
    $id = intval($_GPC['id']);
    ...
    $this->result(0, '', array('branch' => $branch));
}