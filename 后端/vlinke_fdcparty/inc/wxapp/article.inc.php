...
if ($op == "display") {
    ...
    if ($userid != 0 && $article['integral'] > 0 && empty($integral)) {
        $data = array('userid' => $userid, 'channel' => "article", 'foreignid' => $id, 'integral' => $article['integral'], 'remark' => "阅读《" . $article['title'] . "》奖励");
    ...
    $this->result(0, '', array('article' => $article, 'artcate' => $artcate, 'branch' => $branch, 'integralid' => $integralid));
}