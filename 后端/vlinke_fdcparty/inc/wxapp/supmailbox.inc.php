...
if ($op == "display") {
    ...
    $this->result(0, '', array('brancharr' => $brancharr));
} elseif ($op == "post") {
    $data = array('uniacid' => $_W['uniacid'], 'userid' => intval($_GPC['userid']), 'luserid' => intval($_GPC['luserid']), 'title' => trim($_GPC['title']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'details' => trim($_GPC['details']), 'reply' => "", 'status' => 1, 'createtime' => time());
    ...
    $this->result(0, '', array('supmailbox' => $supmailbox, 'luser' => $luser));
}