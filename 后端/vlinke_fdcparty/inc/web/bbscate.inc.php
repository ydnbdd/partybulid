...
if ($op == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        $data = array('uniacid' => $_W['uniacid'], 'branchid' => $branchid, 'name' => trim($_GPC['name']), 'cicon' => trim($_GPC['cicon']), 'ishot' => intval($_GPC['ishot']), 'isshow' => intval($_GPC['isshow']), 'priority' => intval($_GPC['priority']));
        if (!empty($id)) {
            pdo_update($this->table_bbscate, $data, array('id' => $id));
        } else {
            pdo_insert($this->table_bbscate, $data);
        }
        message('信息更新成功！', $this->createWebUrl('bbscate'), 'success');
    }
    ...
    if (empty($bbscate)) {
        $bbscate = array('ishot' => 2, 'isshow' => 1, 'priority' => 0);
    } else {
        $branch = pdo_get($this->table_branch, array('id' => $bbscate['branchid'], 'uniacid' => $_W['uniacid']));
    }
} elseif ($op == 'delete') {
    $id = intval($_GPC['id']);
    $bbscate = pdo_fetch("SELECT * FROM " . tablename($this->table_bbscate) . " WHERE id=:id AND uniacid=:uniacid ", array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($bbscate)) {
        message('要删除的分类不存在或是已经被删除！', referer(), 'error');
    }
    $bbstopic = pdo_getall($this->table_bbstopic, array('cateid' => $id, 'uniacid' => $_W['uniacid']));
    if (!empty($bbstopic)) {
        message('要删除的分类下有话题，请先处理分类下话题再做删除操作！', referer(), 'error');
    }
    pdo_delete($this->table_bbscate, array('id' => $id));
    message('分类信息删除成功！', referer(), 'success');
}
