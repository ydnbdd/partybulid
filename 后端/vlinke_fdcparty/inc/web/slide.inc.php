...
load()->func('tpl');
$positionarr = array('home' => "系统主页", 'arthome' => "党建动态", 'eduhome' => "党员学习", 'exahome' => "在线考试", 'suphome' => "监督执纪", 'serhome' => "志愿服务", 'acthome' => "组织活动", 'malhome' => "积分商城");
if ($op == "display") {
    ...
    if (checksubmit('submit')) {
        ...
        $data = array('uniacid' => $_W['uniacid'], 'position' => trim($_GPC['position']), 'title' => trim($_GPC['title']), 'tilpic' => trim($_GPC['tilpic']), 'link' => trim($_GPC['link']), 'wxapptype' => $wxapptype, 'wxapplink' => $wxapptype == 2 ? urlencode($wxapplink) : $wxapplink, 'priority' => intval($_GPC['priority']));
        if (empty($id)) {
    ...
    if (empty($slide)) {
        $slide = array('position' => 'home', 'link' => 'javascript:;', 'wxapptype' => 1, 'priority' => 0);
    } else {
