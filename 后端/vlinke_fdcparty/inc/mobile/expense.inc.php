...
if ($op == "display") {
    ...
    if (empty($expense) && $expcate['status'] == 3) {
        ...
        $expense = array('uniacid' => $_W['uniacid'], 'cateid' => $cateid, 'userid' => $user['id'], 'paynumber' => date("YmdHis") . rand_str(6, 1), 'paymoney' => $expcate['status'] == 2 ? $expcate['catemoney'] : 0.0, 'paytime' => 0, 'remark' => trim($_GPC['remark']), 'status' => 1, 'createtime' => time());
    ...
    $params = array('tid' => $data['paynumber'], 'ordersn' => $data['paynumber'], 'title' => $expcate['name'], 'fee' => $paymoney);
    $this->pay($params);
    die;
}
