...
if ($op == "display") {
    ...
    if (empty($exaanswer)) {
        ...
        shuffle($arr);
        $exaanswer = array('uniacid' => $_W['uniacid'], 'userid' => $user['id'], 'paperid' => $exapaperid, 'status' => 0, 'aright' => 0, 'awrong' => 0, 'setval' => $exapaper['singlenum'] * $exapaper['singleval'] + $exapaper['multinum'] * $exapaper['multival'], 'getval' => 0, 'integral' => 0, 'stime' => 0, 'etime' => 0, 'finishtime' => 0, 'createtime' => time());
        pdo_insert($this->table_exaanswer, $exaanswer);
        $exaanswer['id'] = pdo_insertid();
        $data = array('uniacid' => $_W['uniacid'], 'userid' => $user['id'], 'itype' => 2, 'foreignid' => $exaanswer['id'], 'myanswer' => "", 'isright' => 0, 'createtime' => time());
        foreach ($arr as $k => $v) {
            $data['bankid'] = $v;
