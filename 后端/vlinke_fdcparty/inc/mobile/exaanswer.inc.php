...
if ($op == "display") {
    ...
    $data = array('status' => 1, 'stime' => time(), 'etime' => time() + 60 * $exapaper['timelimit']);
    ...
    $data = array('status' => 2, 'aright' => count($idrightarr), 'awrong' => count($idwrongarr), 'getval' => $getval, 'integral' => $integral, 'finishtime' => time());
    if ($integral > 0) {
        $intdata = array('userid' => $user['id'], 'channel' => "exaanswer", 'foreignid' => $exaanswer['id'], 'integral' => $integral, 'remark' => $exapaper['title'] . " 考试得分");
