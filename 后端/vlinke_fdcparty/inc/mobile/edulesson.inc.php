...
if ($op == "display") {
    ...
    if (empty($edustudy)) {
        $edustudy = array('uniacid' => $_W['uniacid'], 'userid' => $user['id'], 'lessonid' => $id, 'getval' => 0, 'status' => 1, 'createtime' => time());
    ...
    if ($edulesson['status'] == 2 && $edustudy['status'] == 1 && $nostu_chapterid == 0) {
        ...
        if (empty($integral)) {
            $data = array('userid' => $user['id'], 'channel' => "edustudy", 'foreignid' => $edustudy['id'], 'integral' => $edulesson['integral'], 'remark' => "学习《" . $edulesson['title'] . "》课程");
    ...
    $param['sharetitle'] = $edulesson['title'];
    $param['sharedesc'] = $edulesson['title'];
    $param['sharepic'] = $edulesson['tilpic'];
}
