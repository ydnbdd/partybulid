...
if ($op == "display") {
    ...
    if (!empty($user)) {
        ...
        if ($article['integral'] > 0 && empty($integral)) {
            $data = array('userid' => $user['id'], 'channel' => "article", 'foreignid' => $id, 'integral' => $article['integral'], 'remark' => "阅读《" . $article['title'] . "》奖励");
    ...
    $param['sharetitle'] = $article['title'];
    $param['sharedesc'] = $article['title'];
    $param['sharepic'] = $article['tilpic'];
}
