...
if ($op == "display") {
    ...
    $param['sharetitle'] = $activity['title'];
    $param['sharedesc'] = $activity['title'];
    $param['sharepic'] = $activity['tilpic'];
} elseif ($op == "enroll") {
    ...
    $data = array('uniacid' => $_W['uniacid'], 'activityid' => $activityid, 'userid' => $user['id'], 'utype' => 1, 'getval' => $activity['getval'], 'createtime' => time(), 'signintime' => 0);
    ...
    $url = $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "%26c=entry%26do=activity%26op=sign%26m=vlinke_fdcparty%26activityid=" . $activityid;
} elseif ($op == "setsign") {
    $activityid = intval($_GPC['activityid']);
    $activity = pdo_get($this->table_activity, array('id' => $activityid, 'status' => array(2, 3), 'uniacid' => $_W['uniacid']));
    if (empty($activity)) {
    ...
    if ($activity['getval'] > 0) {
        $intdata = array('userid' => $user['id'], 'channel' => "actenroll", 'foreignid' => $myactenroll['id'], 'integral' => $activity['getval'], 'remark' => "参与《" . $activity['title'] . "》活动奖励");
