...
if ($op == "display") {
    ...
    $param['sharetitle'] = $seritem['title'];
    $param['sharedesc'] = $seritem['title'];
    $param['sharepic'] = $seritem['tilpic'];
} elseif ($op == "claim") {
    ...
    if ($serlogtol < $seritem['unumber']) {
        $data = array('uniacid' => $_W['uniacid'], 'userid' => $user['id'], 'itemid' => $itemid, 'getval' => $seritem['getval'], 'createtime' => time());
        ...
        if ($seritem['getval'] > 0) {
            $intdata = array('userid' => $user['id'], 'channel' => "serlog", 'foreignid' => $id, 'integral' => $seritem['getval'], 'remark' => "认领志愿服务《" . $seritem['title'] . "》奖励");
