...
if ($op == "display") {
    ...
    if (empty($exaday)) {
        ...
        $exaday = array('uniacid' => $_W['uniacid'], 'title' => date('Y') . "年" . date('m') . "月" . date('d') . "日测试", 'userid' => $user['id'], 'status' => 1, 'aright' => 0, 'awrong' => 0, 'integral' => 0, 'finishtime' => 0, 'createtime' => time());
        pdo_insert($this->table_exaday, $exaday);
        $exaday['id'] = pdo_insertid();
        $data = array('uniacid' => $_W['uniacid'], 'userid' => $user['id'], 'itype' => 1, 'foreignid' => $exaday['id'], 'myanswer' => "", 'isright' => 0, 'createtime' => time());
    ...
    $data = array('myanswer' => $myanswer, 'isright' => $myanswer == $exabank['answer'] ? 2 : 1);
    ...
    $data = array('status' => 2, 'aright' => count($idrightarr), 'awrong' => count($idwrongarr), 'integral' => $integral, 'finishtime' => time());
    if ($integral > 0) {
        $intdata = array('userid' => $user['id'], 'channel' => "exaday", 'foreignid' => $exadayid, 'integral' => $integral, 'remark' => $exaday['title'] . "得分");
