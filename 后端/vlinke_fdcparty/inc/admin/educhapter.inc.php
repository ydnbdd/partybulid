...
if ($op == 'display') {
    ...
    if ($_GPC['output'] == 1) {
        ...
        foreach ($list_out as $k => $v) {
            $data[$k] = array('id' => $v['id'], 'lesson' => $edulessonarr[$v['lessonid']]['title'], 'title' => $v['title'], 'needtime' => $v['needtime'], 'status' => $statusarr[$v['status']], 'edulogtol' => intval($edulogtol[$v['id']]['tol']), 'createtime' => date('Y-m-d H:i:s', $v['createtime']) . "\t", 'priority' => $v['priority'], 'link' => $v['link']);
        }
