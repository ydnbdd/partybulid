...
if ($op == 'display') {
    ...
    if ($_GPC['output'] == 1) {
        ...
        foreach ($list_out as $k => $v) {
            $data[$k] = array('id' => $v['id'], 'cateid' => $educate[$v['cateid']]['name'], 'title' => $v['title'], 'integral' => $v['integral'], 'stustatus' => $stustatusarr[$v['stustatus']], 'status' => $statusarr[$v['status']], 'educhaptertol' => intval($educhaptertol[$v['id']]['tol']), 'edustudytol' => intval($edustudytol[$v['id']]['tol']), 'createtime' => date('Y-m-d H:i:s', $v['createtime']) . "\t", 'priority' => $v['priority']);
        }
