    ...
    if (checksubmit('submit')) {
        ...
        if (file_move($tmp, $file_name)) {
            ...
            while ($i <= $len) {
                ...
                if (empty($expense)) {
                    $data = array('uniacid' => $_W['uniacid'], 'cateid' => $expcate['id'], 'userid' => $user['id'], 'paynumber' => date("YmdHis") . rand_str(6, 1), 'paymoney' => $paymoney, 'paytime' => 0, 'remark' => $remark, 'status' => 1, 'createtime' => time());
